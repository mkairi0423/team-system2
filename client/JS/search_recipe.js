...
document.addEventListener("DOMContentLoaded", () => {
    ...
    searchBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            // デバッグ用：データ構造を確認
            console.log("取得データ:", data.ingredients);
 
            let html = `<p>材料と使用量を選んでください:</p>`;
           
            data.ingredients.forEach(item => {
                // 安全に値を取得するために || 0 や || "" を使用
                ...
                html += `
                    <div style="margin: 15px 0; border-bottom: 1px solid #eee; padding-bottom: 10px;">
                        ...
                        <label><strong>${item.food}</strong> (在庫: ${max}${unit})</label>
                        <div style="margin-top: 5px; margin-left: 20px;">
                            ...
                            <span>${unit}</span>
                            <button type="button" onclick="applyRatio(${item.id}, 1)">MAX</button>
                            <button type="button" onclick="applyRatio(${item.id}, 0.5)">半分</button>
                            <button type="button" onclick="applyRatio(${item.id}, 0.33)">1/3</button>
                        </div>
                    </div>`;
            });
